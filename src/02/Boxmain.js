const Boxmain = () => {
    ...
    for(let item of mvlist) {
        ...
        //case5
        let temp = [] ;
        temp = klist.map((k) => <span key={item.movieCd + k} className="col" id={`col${k}`}>{item[k]}</span>); // map은 (k)는 klist의 각 요소를 뜻함,  item[k]를 넣은 새로운 배열을 생성.하여 temp에 할당.
        console.log(temp);
        divTags.push(<div key={item.movieCd} className="rowDiv">{temp}</div>);
        
        /*
        for(let [k, v] of Object.entries(item)){     // for... of 는 key만 가져옴, key,value 같이 가져오고싶을때 Object.entries(item) 사용.
            // case1
        ...
        */
            // case3
            //for(let k of klist) console.log(k, item[k]);
            /*
            // case4
    ...
    // for(let item in mvlist) console.log(mvlist[item]); // 위의 for...of와 같은 코드.
    ...
    return(
        <div className = "content">
            <div className = "rowDiv">
                ...
                <span className = "col" id="colrankInten">증감율</span>
            </div>
            {divTags}
        </div>
    );
